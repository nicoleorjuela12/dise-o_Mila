...
const BarraCliente = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      if (response.status === 200) {
        localStorage.clear(); // Eliminar todos los datos relacionados con el usuario
  ...
  return (
    <div className="flex flex-col items-center justify-center mb-24">
          ...
          <div className="relative">
            ...
            {showUserMenu && (
              <div id="user-menu" className="absolute right-0 mt-2 w-48 py-2 bg-white border border-gray-200 rounded-lg shadow-lg">
                ...
                <button onClick={handleLogout} className="block w-full px-4 py-2 text-red-700 hover:bg-yellow-200 no-underline">Cerrar sesión</button>
              </div>
            )}
          </div>
        </div>
      ...
      {showMobileMenu && (
        <div id="mobile-menu" className="fixed inset-0 bg-white z-50 p-6">
          <button
            className="p-2 mb-4 text-black-500 rounded-lg hover:bg-yellow-200 focus:outline-none focus:ring-2 focus:ring-yellow-200"
          ...
          <Link to="/DashboardCliente" className="block py-2">Inicio</Link>
          <Link to="/productos" className="block py-2">Productos</Link>
          <Link to="/pedidoss" className="block py-2">Pedidos</Link>
          <Link to="/RegistroEventosCliente" className="block py-2">Eventos</Link>
          <Link to="/perfilusuario" className="block py-2">Perfil</Link>
          <button onClick={handleLogout} className="block py-2 text-red-600">Cerrar sesión</button>
        </div>
      )}
    </div>
  );
