...
const PerfilUsuario = () => {
  ...
  const checkIfDataExists = async () => {
    try {
      const response = await axios.get('http://localhost:8000/usuarios/verificar-usuarios', {
        params: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        const id_usuario = localStorage.getItem('id_usuario');
        await axios.put(`http://localhost:8000/usuarios/${id_usuario}`, formData);
        Swal.fire({
          title: 'Éxito',
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="form-group relative">
            ...
            <input
              ...
              onChange={handleChange}
              className={`mt-1 p-3 border ${errors.correo ? 'border-red-500' : 'border-yellow-500'} rounded-lg w-full focus:ring-yellow-500 focus:border-yellow-500 transition duration-150 ease-in-out`}
            />
